...
describe('TaskStatus', () => {
  describe('valid transitions', () => {
    it('Todo → In Progress', () => {
      expect(TaskStatus.TODO.moveTo(TaskStatus.IN_PROGRESS)).toBe(
        TaskStatus.IN_PROGRESS,
      );
    });

    it('In Progress → Done', () => {
      expect(TaskStatus.IN_PROGRESS.moveTo(TaskStatus.DONE)).toBe(
        TaskStatus.DONE,
      );
    });
  });
  ...
  describe('from()', () => {
    ...
    it('throws on unknown value', () => {
      expect(() => TaskStatus.from('unknown')).toThrow(
...
describe('TaskService', () => {
  ...
  describe('create()', () => {
    ...
    it('does not publish any event when the repository throws', async () => {
      ...
      await expect(
        service.create({ title: 'Fix bug', projectId: 10 }),
      ).rejects.toThrow();
  ...
  describe('moveTask()', () => {
    ...
    it('throws NotFoundException when task does not exist', async () => {
      ...
      await expect(
        service.moveTask(99, TaskStatus.IN_PROGRESS),
