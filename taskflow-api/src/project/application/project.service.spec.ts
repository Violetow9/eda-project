...
const makeProject = (overrides: Partial<ConstructorParameters<typeof Project>[0]> = {}): Project =>
    new Project({id: 1, projectName: 'TaskFlow', members: [], ...overrides});

describe('Project entity', () => {
    it('throws when project name is empty', () => {
        expect(() => makeProject({projectName: ''})).toThrow('Project name cannot be empty');
        expect(() => makeProject({projectName: '   '})).toThrow('Project name cannot be empty');
    });
...
describe('ProjectService', () => {
    ...
    describe('addMember()', () => {
        ...
        it('throws NotFoundException when project does not exist', async () => {
            ...
            await expect(service.addMember(99, 'user-1')).rejects.toThrow(NotFoundException);
